(() => {
  ...
  function renderHistory() {
    ...
    historyList.innerHTML = state.flips.length
      ? state.flips.map((entry) => `<div class="history-card"><strong>${entry.side}</strong><span class="muted">${entry.time}</span></div>`).join("")
      : '<div class="history-card"><strong>Abhi koi toss nahi hua</strong><span class="muted">Pehla flip karke streak shuru karo.</span></div>';
  }
  ...
  function flipCoin() {
    ...
    window.setTimeout(() => {
      ...
      const stamp = new Intl.DateTimeFormat("en-IN", { hour: "numeric", minute: "2-digit" }).format(new Date());
